...
| <a id="apple_xcframework-include_dsym"></a>include_dsym |  If true, include dSYM bundles inside the generated XCFramework and record `DebugSymbolsPath` in the XCFramework's Info.plist for each library slice. This mimics `xcodebuild -create-xcframework -debug-symbols` behavior to ship debug symbols with distribution artifacts.   | Boolean | optional |  `False`  |
