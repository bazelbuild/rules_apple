...
| <a id="ios_xctestrun_runner-attachment_lifetime"></a>attachment_lifetime |  Attachment lifetime to set in the xctestrun file when running the test bundle - `"keepNever"` (default), `"keepAlways"` or `"deleteOnSuccess"`. This affects presence of attachments in the xcresult output. This does not force using `xcodebuild` or an XCTestRun file but will be used in that case.   | String | optional |  `"keepNever"`  |
