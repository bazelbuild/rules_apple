...
| <a id="ios_application-include_symbols_in_bundle"></a>include_symbols_in_bundle |  Retained for compatibility. Application bundles no longer package `$UUID.symbols` files directly. To include generated symbols in a distributable archive, wrap this target in `apple_archive` and set `include_symbols = True`.   | Boolean | optional |  `False`  |
