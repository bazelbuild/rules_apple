...
parse_args() {
  ...
  test_platform_type="%(test_platform_type)s"
  test_minimum_os_version="%(test_minimum_os_version)s"
  ...
  while [[ $# -gt 0 ]]; do
    ...
    case $arg in
      ...
      --destination=platform=*,id=*)
        ...
        destination_platform="${destination_platform%%,id=*}"
...
platform_display_name() {
  case "$test_platform_type" in
    ...
    watchos)
      ...
      ;;
    *)
      echo "error: unsupported Apple test platform '$test_platform_type'" >&2
      exit 1
      ;;
  esac
}

simulator_platform_directory_name() {
  case "$test_platform_type" in
    ios)
      echo "iPhoneSimulator.platform"
      ;;
    tvos)
      echo "AppleTVSimulator.platform"
      ;;
    macos)
      echo "MacOSX.platform"
      ;;
    visionos)
      echo "XROSSimulator.platform"
      ;;
    watchos)
      echo "WatchSimulator.platform"
      ;;
    *)
      echo "error: unsupported Apple simulator platform '$test_platform_type'" >&2
      exit 1
      ;;
  esac
}

device_platform_directory_name() {
  case "$test_platform_type" in
    ios)
      echo "iPhoneOS.platform"
      ;;
    tvos)
      echo "AppleTVOS.platform"
      ;;
    macos)
      echo "MacOSX.platform"
      ;;
    visionos)
      ...
      ;;
    watchos)
      echo "WatchOS.platform"
      ;;
    *)
      echo "error: unsupported Apple device platform '$test_platform_type'" >&2
      exit 1
      ;;
  esac
}
...
platform_sdk_version() {
  case "$test_platform_type" in
    ios)
      echo "%(ios_sdk_version)s"
      ;;
    tvos)
      echo "%(tvos_sdk_version)s"
      ;;
    macos)
      echo "%(macos_sdk_version)s"
      ;;
    visionos)
      echo "%(visionos_sdk_version)s"
      ;;
    watchos)
      echo "%(watchos_sdk_version)s"
      ;;
    *)
      echo "error: unsupported Apple SDK platform '$test_platform_type'" >&2
      exit 1
      ;;
  esac
}
...
prepare_work_dir() {
  ...
  profraw="$test_tmp_dir/coverage.profraw"
  if [[ -z "${NO_CLEAN:-}" ]]; then
...
stage_test_bundle() {
  ...
  if [[ "$test_platform_type" == "macos" ]]; then
...
select_execution_platform() {
  ...
  test_execution_platform="$(simulator_platform_directory_name)"
  xcodebuild_destination_platform="$(platform_display_name)"
  if [[ "$test_platform_type" == "macos" ]]; then
    test_execution_platform="MacOSX.platform"
    xcodebuild_destination_platform="macOS"
    return
  fi
  if [[ -n "$device_id" ]]; then
    test_execution_platform="$(device_platform_directory_name)"
    xcodebuild_destination_platform="${destination_platform:-$xcodebuild_destination_platform}"
...
build_command_line_args_section() {
...
build_test_environment() {
  ...
  if [[ "$test_platform_type" == "macos" && "${COVERAGE:-}" -eq 1 ]]; then
    default_test_env="$default_test_env,LLVM_PROFILE_FILE=$profraw"
  fi
...
configure_test_host() {
  ...
  if [[ "$test_platform_type" == "macos" ]]; then
    ...
    if [[ -n "$test_host_path" ]]; then
      ...
      xctestrun_env+="<key>XCInjectBundleInto</key><string>$(escape "__TESTHOST__/Contents/MacOS/$test_host_name")</string>"
    else
  ...
  if [[ "$test_type" = "XCUITEST" ]]; then
    ...
    if [[ "$test_platform_type" == "ios" ]]; then
      # We need this dylib for 14.x OSes. This intentionally uses the device
      # platform since this file isn't present in the simulator platform.
      # No longer necessary starting in Xcode 15 - hence the `-f` file existence check.
      libswift_concurrency_path="$developer_dir/Platforms/$(device_platform_directory_name)/Library/Developer/CoreSimulator/Profiles/Runtimes/iOS.simruntime/Contents/Resources/RuntimeRoot/usr/lib/swift/libswift_Concurrency.dylib"
...
configure_injected_libraries() {
  if [[ "$test_platform_type" == "macos" ]]; then
    xctestrun_libraries="%(macos_xctestrun_insert_libraries)s"
    return
  fi
...
configure_platform_environment() {
  if [[ "$test_platform_type" == "macos" ]]; then
    xctestrun_platform_environment="<key>DYLD_FALLBACK_LIBRARY_PATH</key><string>__PLATFORMS__/MacOSX.platform/Developer/usr/lib</string>"
    xctestrun_platform_environment+="<key>DYLD_FRAMEWORK_PATH</key><string>__PLATFORMS__/MacOSX.platform/Developer/Library/Frameworks:__SHAREDFRAMEWORKS__</string>"
    xctestrun_platform_environment+="<key>DYLD_LIBRARY_PATH</key><string>__PLATFORMS__/MacOSX.platform/Developer/Library/Frameworks:__SHAREDFRAMEWORKS__</string>"
    return
  fi

  xctestrun_platform_environment="<key>DYLD_LIBRARY_PATH</key><string>__PLATFORMS__/$test_execution_platform/Developer/usr/lib</string>"
}
...
create_simulator() {
  ...
  if [[ "$build_for_device" == false ]]; then
    simulator_id="$(SIMULATOR_PLATFORM_TYPE="$test_platform_type" SIMULATOR_DEVICE_TYPE="%(device_type)s" SIMULATOR_OS_VERSION="%(os_version)s" SIMULATOR_REUSE_SIMULATOR="${reuse_simulator:-}" SIMULATOR_SDK_VERSION="$(platform_sdk_version)" XCTESTRUN_RUNNER_PID="${BASHPID:-$$}" "%(create_simulator_action_binary)s")"
  fi
}

choose_execution_strategy() {
  test_exit_code=0
  testlog=$test_tmp_dir/test.log
  if [[ "$test_platform_type" == "macos" ]]; then
...
run_with_xcodebuild() {
  ...
  xctestrun_file="$test_tmp_dir/tests.xctestrun"
  /usr/bin/sed \
    ...
    -e "s${sed_delim}BAZEL_PLATFORM_ENVIRONMENT${sed_delim}$xctestrun_platform_environment${sed_delim}g" \
  ...
  if [[ "$test_platform_type" == "macos" ]]; then
    args+=(-destination "platform=macOS,variant=macos,arch=$architecture")
  elif [[ "$build_for_device" == true ]]; then
    args+=(-destination "platform=$xcodebuild_destination_platform,id=$device_id")
  else
    args+=(-destination "id=$simulator_id")
  fi

  result_bundle_path="$TEST_UNDECLARED_OUTPUTS_DIR/tests.xcresult"
  # TEST_UNDECLARED_OUTPUTS_DIR isn't cleaned up with multiple retries of flaky tests
  rm -rf "$result_bundle_path"
  if [[ "$create_xcresult_bundle" == true || "$test_platform_type" == "macos" ]]; then
...
run_with_xctest() {
  if [[ "$test_platform_type" == "macos" ]]; then
    echo "error: macOS tests require xcodebuild" >&2
    exit 1
  fi
...
fail_for_post_action_before_cleanup() {
