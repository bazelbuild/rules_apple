"""Apple test runner rule that uses xctestrun files."""
...
def _get_template_substitutions(
    ...
    substitutions = {
        ...
        "macos_xctestrun_insert_libraries": macos_xctestrun_insert_libraries,
        ...
        "screen_capture_format": screen_capture_format,
...
def _get_execution_environment(ctx):
    xcode_version = ctx.attr._xcode_config[XcodeVersionInfo].xcode_version()
    if not xcode_version:
        return {}

    return {"XCODE_VERSION_OVERRIDE": str(xcode_version)}
...
def _test_platform_type(ctx):
    single_arch_platform = getattr(ctx.fragments.apple, "single_arch_platform", None)
    if single_arch_platform:
        return str(single_arch_platform.platform_type)

    platform_type = getattr(ctx.fragments.apple, "platform_type", None)
    if platform_type:
        return str(platform_type)

    fail("Unable to derive the Apple test platform for {}".format(ctx.label))

def _apple_xctestrun_runner_impl(ctx):
    ...
    ctx.actions.expand_template(
        ...
        substitutions = _get_template_substitutions(
            ...
            macos_sdk_version = str(xcode_properties.default_macos_sdk_version),
...
apple_xctestrun_runner = rule(
    _apple_xctestrun_runner_impl,
    attrs = {
...
""",
        ...
        "screen_capture_format": attr.string(
            default = "",
