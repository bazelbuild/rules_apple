...
<li>`SIMULATOR_POOL_SLOT`: The simulator pool slot this test action claimed, as a non-negative integer. The runner gives each concurrently-running test an exclusive slot so no two live tests share a simulator; slot "0" names the simulator the runner has historically reused, higher slots only occur while tests actually run concurrently and should map to distinct simulators (the default creator appends "_N" to the simulator name). Unset when the invoking runner does not participate in the pool, which should be treated the same as "0".</li>
