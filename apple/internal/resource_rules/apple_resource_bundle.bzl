...
apple_resource_bundle = rule(
    ...
    attrs = {
        "bundle_name": attr.string(
            doc = """
The desired name of the bundle (without the `.bundle` extension).

If this attribute is not set, then the `name` of the target will be used instead.
""",
        ),
        "infoplists": attr.label_list(
            ...
            allow_files = True,
            doc = """
Infoplist files (`.plist`) to be merged into the bundle's `Info.plist`.

Duplicate keys between infoplist files will cause an error if and only if the values conflict.

Please see [`Info.plist` handling](common_info.md#infoplist-handling) for what is supported.
""",
        ),
...
level Apple bundle dependent. `apple_resource_bundle` targets need to be added
to library targets through the `data`, `deps` or `private_deps` attributes.
""",
)
