...
def generate_app_intents_metadata_bundle(
    ...
    Args:
        ...
        direct_app_intents_modules: List of structs from the return value of previous invocations of
            generate_app_intents_metadata_bundle(...) representing Swift modules that are direct
    ...
    # When querying for embedded metadata bundles, make sure to single out only the ones that will
    # appear in the final bundle, mirroring Xcode. Otherwise, we will pick up the metadata bundles
    # for intermediate static library outputs, which Xcode does not do.
    embedded_metadata_bundle_files = [
        p.bundle
        for x in embedded_metadata_bundles
        for p in x.to_list()
        if p.should_include_in_bundle
    ]
    if embedded_metadata_bundle_files:
    ...
    return struct(
        ...
        module_name = intents_module_name,
        owner = owner,
        should_include_in_bundle = main_bundle_output,
    )
