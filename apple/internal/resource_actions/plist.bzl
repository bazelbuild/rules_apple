...
def merge_resource_infoplists(ctx, bundle_name, input_files, output_plist):
    ...
    prefix = ctx.attr.bundle_id if hasattr(ctx.attr, "bundle_id") else _derive_bundle_id_prefix(ctx.label)
    substitutions["PRODUCT_BUNDLE_IDENTIFIER"] = prefix + "." + bundle_name
