...
function test_kernel_extension() {
  ...
  cat > kext/kext-builder_info.c <<EOF
#include <mach/mach_types.h>

extern kern_return_t _start(kmod_info_t *ki, void *data);
extern kern_return_t _stop(kmod_info_t *ki, void *data);

__attribute__((visibility("default"))) KMOD_EXPLICIT_DECL(com.google.kext_builder, "1.0.0d1", _start, _stop)
...
EOF
  ...
  cat > kext/BUILD <<EOF
load("@build_bazel_rules_apple//apple:macos.bzl", "macos_bundle", "apple_product_type")
