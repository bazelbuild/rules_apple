...
# Integration tests for bundling simple iOS UI tests.
...
#
# Creates a minimal iOS application target along with a minimal UI Test
...
function create_ios_ui_test_without_test_host() {
  if [[ ! -f app/BUILD ]]; then
    fail "create_common_files must be called first."
  fi
...
function test_plist_contents() {
  ...
  create_dump_plist "//app:ui_tests_test_bundle" "ui_tests.xctest/Info.plist" \
      BuildMachineOSBuild \
      CFBundleExecutable \
      CFBundleIdentifier \
      CFBundleName \
