...
bundles are present.""",
    ...
    # load command LC_BUILD_VERSION for the arm64 binary slice when only iOS cpus are defined, and
    # that it does not default to an unsupported architecture.
    binary_contents_test(
    ...
    archive_contents_test(
        name = "{}_drops_arm64_32_for_minimum_os_27_or_higher_test".format(name),
        build_type = "device",
        target_under_test = "//test/starlark_tests/targets_under_test/watchos:app_minimum_os_27",
        cpus = {"watchos_cpus": ["device_arm64", "arm64_32"]},
        binary_test_file = "$BUNDLE_ROOT/app_minimum_os_27",
        binary_test_architecture = "arm64",
        binary_not_contains_architectures = ["arm64_32"],
