...
Expected Apple platform type of \"{0}\", but that was not found in //buildenv/platforms/apple/simulator:watchos_x86_64.
...
analysis_incoming_watchos_platform_mismatch_test = analysistest.make(
    ...
    config_settings = {
        ...
        "//command_line_option:platforms": ["//buildenv/platforms/apple/simulator:watchos_x86_64"],
    },
)
...
def _analysis_incoming_ios_platform_mismatch_test_impl(ctx):
    ...
    asserts.expect_failure(env, """
ERROR: Unexpected resolved platform:
Expected Apple platform type of \"{0}\", but that was not found in //buildenv/platforms/apple/simulator:ios_arm64.
""".format(ctx.attr.expected_platform_type))
    return analysistest.end(env)
