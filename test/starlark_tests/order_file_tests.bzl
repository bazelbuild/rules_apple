...
load("//tools/build_defs/write_file:write_file.bzl", "write_file")
...
def _test_provider_contents(*, name, tags):
    ...
    write_file(name = order_file_a, out = "{}.order".format(order_file_a), content = "content_a")
    order_file_b = "{}_b".format(name)
    write_file(name = order_file_b, out = "{}.order".format(order_file_b), content = "content_b")
