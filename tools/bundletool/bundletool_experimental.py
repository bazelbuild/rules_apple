...
class PostProcessorError(EnvironmentError):
    ...
    def __init__(self, exit_code):
        self.exit_code = exit_code
        EnvironmentError.__init__(self, POST_PROCESSOR_ERROR_MSG_TEMPLATE % exit_code)
...
class Bundler(object):
        ...
        if os.path.islink(src) or os.path.isfile(src):
            ...
            for root, dirs, files in os.walk(src):
                ...
                for dirname in list(dirs):
                    fsrc = os.path.join(root, dirname)
                    if not os.path.islink(fsrc):
                        continue
                    fdest = os.path.normpath(os.path.join(dest, relpath, dirname))
                    self._copy_file(
                        fsrc,
                        fdest,
                        executable,
                ...
                for filename in files:
                    ...
                    self._copy_file(
                        ...
                        executable,
                        bundle_root,
                        preserve_symlink=os.path.islink(fsrc),
                    )
    ...
    def _copy_file(self, src, dest, executable, bundle_root, preserve_symlink=False):
        ...
        Args:
          ...
          bundle_root: The bundle root directory into which the files should be
              added.
          preserve_symlink: If True, symbolic links are copied as links instead of
              copying the linked file's contents.
        """
        full_dest = os.path.join(bundle_root, dest)
        if preserve_symlink and os.path.islink(src):
            link_target = os.readlink(src)
            if os.path.lexists(full_dest):
                if os.path.islink(full_dest) and os.readlink(full_dest) == link_target:
                    return
                raise BundleConflictError(dest)

            self._makedirs_safely(os.path.dirname(full_dest))
            os.symlink(link_target, full_dest)
            return

        if os.path.isfile(full_dest) and not filecmp.cmp(full_dest, src, shallow=False):
            raise BundleConflictError(dest)

        self._makedirs_safely(os.path.dirname(full_dest))
        global _USE_CLONEFILE
        if _USE_CLONEFILE and not os.path.islink(src):
            ...
            shutil.copy(src, full_dest)
        os.chmod(full_dest, 0o755 if executable or os.access(src, os.X_OK) else 0o644)
    ...
    def _sign_bundle(self, bundle_root, command_lines):
        ...
        for command in command_lines.splitlines():
            argv = [
                arg.replace("$WORK_DIR", bundle_root) for arg in shlex.split(command)
            ]
            try:
