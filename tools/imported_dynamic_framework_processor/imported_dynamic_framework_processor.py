...
def _try_get_framework_version_from_structure(
...
def _relpath_from_framework(framework_absolute_path):
    ...
    if parent_dir == "/":
        raise ValueError(
            "Internal Error: Could not find path in framework: "
...
def _strip_framework_binary(framework_binary, output_path, slices_needed):
    ...
    if not slices_needed:
        raise ValueError(
            "Internal Error: Did not specify any slices needed for binary at "
...
def _strip_or_copy_binary(
    *,
    ...
    # operation.
    should_skip_lipo = len(binary_archs) == 1 or binary_archs == set(requested_archs)

    if should_skip_lipo:
        binary_path = _copy_framework_file(
            framework_binary, executable=True, output_path=output_path
        )
    else:
        binary_path = _strip_framework_binary(
            framework_binary, output_path, slices_needed
        )
...
def _get_parser():
    ...
    parser = argparse.ArgumentParser(description="imported dynamic framework processor")
    ...
    parser.add_argument(
        ...
        required=True,
        action="append",
        help="binary slice " "expected to represent the target architectures",
    )
    parser.add_argument(
        "--strip_bitcode",
        action="store_true",
        default=False,
        help="strip " "bitcode from the imported frameworks.",
    )
    parser.add_argument(
        "--framework_file",
        type=str,
        ...
        action="append",
        help=(
            "path to a file scoped to one of the imported"
    ...
    parser.add_argument(
        "--temp_path",
        type=str,
        help="temporary path to copy all framework "
        "files to before creating an output zip",
    )
    output_group = parser.add_mutually_exclusive_group(required=True)
    output_group.add_argument(
        "--output_zip",
        type=str,
        help="path to save the zip file containing a "
        "codesigned, lipoed version of the imported framework",
    )
    output_group.add_argument(
        "--output_directory",
        type=str,
        help="path to the directory where the "
        "codesigned, lipoed version of the imported framework should be written",
    )
    parser.add_argument(
        "--disable_signing",
        action="store_true",
...
def _framework_output_path(args, parser) -> str:
    """Returns the directory where the processed framework should be assembled."""
    if args.output_zip:
        if not args.temp_path:
            parser.error("--temp_path is required when --output_zip is used.")
        return args.temp_path

    return args.output_directory


def _prepare_output_paths(output_path: str, output_zip: Optional[str]) -> None:
    """Clears stale outputs and creates the processed framework output directory."""
...
def _archive_processed_framework(output_path: str, output_zip: str) -> None:
    """Archives the processed framework directory into a zip file."""
...
def main() -> None:
    ...
    output_path = _framework_output_path(args, parser)

    # Delete any existing stale framework files, if any exist.
    _prepare_output_paths(output_path=output_path, output_zip=args.output_zip)
    ...
    if not is_versioned_framework:
        ...
        for framework_file in args.framework_file:
            _copy_framework_file(
                framework_file, executable=False, output_path=output_path
            )
    else:
        ...
        symlink_path = os.path.join(output_path, "Versions", "Current")
        ...
        versions_dir = os.path.join(output_path, "Versions")
        version_dir = os.path.join(versions_dir, version)
        for entry in os.listdir(version_dir):
            symlink_path = os.path.join(output_path, entry)
    ...
    _update_modified_timestamps(output_path)
    ...
    if args.output_zip:
        _archive_processed_framework(
            output_path=output_path, output_zip=args.output_zip
        )
