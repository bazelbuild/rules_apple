...
def _is_versioned_file(filepath: str) -> bool:
  """Returns True if file is below a Darwin framework versioned file."""
  return ".framework/Versions/" in filepath


def _is_current_file(filepath: str) -> bool:
  """Returns True if file is below a Darwin framework current version file."""
  return ".framework/Versions/Current/" in filepath
...
def _get_install_path_for_binary(binary: str) -> str:
  ...
  result = re.search(r".*(@rpath/.*)", stdout)
...
def _get_framework_version_from_install_path(binary: str) -> str:
  ...
  result = re.search(version_regex, install_path)
  if not result or not result.groups():
    raise ValueError(
        textwrap.dedent(f"""\
    ...
    for root, dirs, files in os.walk(framework_temp_path, topdown=False):
      for file_name in dirs + files:
        ...
        if os.path.islink(file_path):
          ...
          # It's safe to asume so because Bazel resolves symlinks.
          continue
...
def main() -> None:
  ...
  if not is_versioned_framework:
    ...
    for framework_file in args.framework_file:
      if (_is_versioned_file(framework_file) and
          not _is_current_file(framework_file)):
    ...
    # Modify codesign args to sign each framework version
    targets_to_sign = []
    for version_dir in os.listdir(versions_dir):
      if version_dir == "Current":
        continue
      targets_to_sign.append(os.path.join(versions_dir, version_dir))
    args.target_to_sign = targets_to_sign
