...
"""Wrapper for "xcrun momc".

"momcwrapper" runs "xcrun momc", working around issues with relative paths. This
script only runs on Darwin and you must have Xcode installed.

  momcwrapper [OUTPUT] [<args>...]

  OUTPUT: The path to the desired output file (.mom) or directory (.momd),
      depending on whether or not the input is a versioned data model.
  <args>: Additional arguments to pass to "momc".
"""
...
def _main():
  ...
  xctoolrunner.main(["momc"] + sys.argv[1:])
